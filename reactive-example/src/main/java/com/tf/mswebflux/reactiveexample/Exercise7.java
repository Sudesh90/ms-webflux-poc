...
//				.skip(3).take(3).log().subscribe(System.out::println);
...
//		ReactiveSources.intNumbersFlux().filter(e -> e > 20).defaultIfEmpty(-1).subscribe(System.out::println);
		...
		// -1 since there is no element greater than 20

		// Switch ints from intNumbersFlux to the right user from userFlux
//		ReactiveSources.intNumbersFlux()
//			.flatMap(id -> ReactiveSources.userFlux().filter(user -> user.getId() == id))
//			.take(1) // User{id=1, firstName='Lionel', lastName='Messi'} - it will take one and print
//			.subscribe(System.out::println);

		// Print only distinct numbers from intNumbersFluxWithRepeat
//		ReactiveSources.intNumbersFluxWithRepeat()
//			.distinct() //1 2 3 4 5 
//			.distinctUntilChanged() //1, 2, 4, 5, 5, 1 (here it found 1 again)
//			.log()
//			.subscribe(System.out::println);
